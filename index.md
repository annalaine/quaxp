...
On this page, we present in different sections, the parts the book contains and how they should be used, the two different difficulty levels (beginner and advanced), and their content.
...
In this chapter, we explore how text data can be preprocessed and cleaned and how words can be used by a machine learning algorithm. The example task we use is a sentiment analysis, or opinion mining, using tweets from Twitter. Sentiment analysis gives an idea on how a concept is perceived by the public from tweets, or other texts like reviews, classifying them into positive or negative categories according to their contents.
...
Tu run the cells, click on the rocket logo on the top right of the page (this logo only appears on pages that contain code cells). Then, two options are possible:
+ Select "Live Code".  This allows you to run and modify the code cells directly on the page. It may take some time to get ready, so I suggest you start it as soon as you are on the page.
+ Select "Binder". This will redirect you to another page: the page of the book will be open with Jupyter Notebook in your browser. You might see some hidden content in this version, do not pay attention to those cells (the hidden cells are marked as "cell to hide"). At the end of the page, you need to return manually to the book to see the menu and switch pages.
